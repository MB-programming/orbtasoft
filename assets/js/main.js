document.addEventListener('DOMContentLoaded', function () {
  ...
  if (workFilter && workTrack) {
    workFilter.addEventListener('click', function (e) {
      var btn = e.target.closest('.work-filter__btn');
      if (!btn) return;

      workFilter.querySelectorAll('.work-filter__btn').forEach(function (b) { b.classList.remove('is-active'); });
      btn.classList.add('is-active');

      var filter = btn.getAttribute('data-filter');
      workTrack.querySelectorAll('.work-card').forEach(function (card) {
